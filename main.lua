...
local function applySpeedHack(char)
    ...
    if hum then
        if state.speedHack then
            hum.WalkSpeed = (state.walkSpeed or 16) * (state.speedMultiplier or 2)
        else
...
MainTab:CreateButton({Name="Rejoin Server", Callback=function() local jobId = HttpService:GenerateGUID(false) pcall(function() game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer) end) end})
